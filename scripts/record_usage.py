...
def update_readme(rows: dict[str, dict], snap: dict, today: str) -> None:
    ...
    # 🔴 `or "—"` 로 쓰면 **다운로드 0 이 '미상'(—)으로 둔갑한다**(0 은 falsy).
    #    이 저장소들이 스스로 지키는 원칙이 '0건과 미상을 섞지 않는다' 인데 표시가
    #    그것을 어기고 있었다. 자매 저장소는 다운로드가 3이라 드러난 적이 없고,
    #    릴리스가 아직 없는 새 저장소로 이식하며 보였다.
    _dl = snap.get("release_downloads")
    dl = f"{int(_dl):,}" if str(_dl).strip() not in ("", "None") else "—"
